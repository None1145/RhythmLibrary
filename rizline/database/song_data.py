...
class SongData:
    ...
    def add_song(self, id: str, title: str,
    ...
    def get_songs_level_range(self, song_level_min: float, song_level_max: float) -> List[Dict[str, Union[str, List[str]]]]:
        session = self.Session()
        try:
            results = []
            ...
            for level_data in all_levels:
                ...
                for level_name in ["EZ", "HD", "IN", "AT", "SP"]:
                    level_info = levels_json.get(level_name, {})
                    level_num = level_info.get("num", 0)
                    
                    if level_num != 0 and song_level_min <= level_num <= song_level_max:
